...
const QuestionCard = () => {
    const { todayQuestion, setTodayQuestion } = useQuestionContext()

    // ฟังก์ชันเลือกคำถามตามวัน
    const getQuestionOfToday = () => {
        const today = new Date().toISOString().split("T")[0] // เช่น "2025-10-08"
        let seed = 0
    ...
    return (
        <div className="w-full flex flex-col items-center justify-center gap-6 mt-10">
            ...
            <div className="w-[90%] md:w-[70%] bg-white/10 rounded-md py-8 border border-[#804cb2] text-[#e6d5f7] text-[20px] flex justify-center items-center text-center">
                {todayQuestion?.question || "กำลังโหลดคำถาม..."}
            </div>
